...
class Recharge extends UserCommon{
    public function index(){
        ...
        if ($order_num||$id||$trade_no){
            ...
            $where8 = 1;
            if ($order_num) {
                $where5['order_num'] = ['like'.'%'.$order_num.'%'];
                $where6 = 1;
                $where10 = 1;
            }elseif($trade_no){
                $where10['trade_no'] = ['like'.'%'.$trade_no.'%'];
                $where6 = 1;
                $where5 = 1;
            }else{
                $where5 = 1;
                ...
                if (!isset($id_arr[1])) {
                    ...
                    $where6['m.id'] = ['like','%'.$id_arr[0].'%'];
                }else{
                    $where6['m.id'] = ['like','%'.$id_arr[1].'%'];
                }
            }
        }else{
      	...
      	if($channel_id){
        	$where11['m.type'] = $channel_id;
        }else{
        	$where11 = '';
        }

        $where11['m.proxy_id'] = $this->login_user['id'];
        ...
        $top_account_assets = Db::table('top_account_assets')
          ...
          ->select();
      	$channel_data = Db::table('user_fee')
         ->alias('u')
         ->field('u.taid,c.channel_name')
         ->join('channel_type c','c.code=u.taid')
         ->where('uid',$this->login_user['id'])
         ->select();
