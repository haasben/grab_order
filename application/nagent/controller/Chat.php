...
class Chat extends UserCommon
{
    ...
    */
    public function index() {
        ...
        $user_data = session('login_user','','nagent');
        if(!empty($user_data)){
            $username = '客户'.$order_id;
            $uid = $user_data['id'];
            $is_kefu = 1;
          	Gateway::$registerAddress = '127.0.0.1:1238';
  			Gateway::sendToUid($uid, json_encode(['type'=>'has_done']));
        }else{
    ...
    {
        if (!request()->isAjax()) {
            return $this->redirect('nagent/Index/index');
        }
        Gateway::$registerAddress = '127.0.0.1:1238';
        ...
        $magent = session('login_user','','nagent');
        // dump($magent);die;
        if(empty($magent)){
