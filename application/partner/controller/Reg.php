...
class Reg extends Controller{
    ...
    public function register_code_ajax(){
		...
		if ($validate->check($data)){
         ...
         // $bool = 1;
			session('phone_num',$data['phone_num']);
    ...
    public function register_3(){
    	if (request()->isPost()) {
			...
			if ($insert_id) {
				...
				$email_str = '<strong>尊敬的商户，您好！ </strong> </br>&nbsp;&nbsp;&nbsp;&nbsp;请您立刻使用下面的链接来激活您的账号。<a href="http://'.$_SERVER['HTTP_HOST'].'/index/Registered/email_code?email='.$data['email'].'&email_code='.$email_code.'">点击验证</a>。</br>如果因为您的邮件阅读程序不支持点击跳转验证，建议您把以下链接复制到网页浏览器（如：IE）的地址栏里打开：</br>http://'.$_SERVER['HTTP_HOST'].'/index/Registered/email_code?email='.$data['email'].'&email_code='.$email_code.'</br>注：这是一封系统自动产生的邮件，请勿回复。</br><div style="COLOR: #000; text-align:right;">红云集成</div><div style="text-align:right;">'.date('Y-m-d').'</div>';
    ...
    public function email_code(){
		...
		$data['pass'] = encryption($data['pass']);
      	...
      	$data['key'] = md5($data['authenticator']);
       
		$invite = input('id');
        if($invite != 1){
        	$data['superior'] = $invite;
        }
		...
		if($invite != 1){
          	...
          	$invite_user = Db::table('users')->where('id',$invite)->find();
          	if($invite_user['state'] == 89){
            	$subordinate = $invite_user['subordinate'];
                $subordinate = ltrim($subordinate.','.$insert_id,',');
                $bool3 = Db::table('users')->where('id', $invite)->update(['subordinate' => $subordinate]);
            }
      		

      	}

		$assets = ['uid' => $insert_id];
		...
		$fee = Db::table('channel_type')->field('id,fee')->select();

		foreach ($fee as $k => $v) {
			
			$insert_data[] = ['uid'=>$insert_id,'taid'=>$v['id'],'fee'=>$v['fee']];

		}
		$boo12 = Db::table('user_fee')->insertAll($insert_data);

		if($boo12&&$insert_id&&$bool3){
			Db::commit();
			$ret['success'] = '0000';
			$ret['hint'] = '注册成功，马上登陆';
