...
class PaymentAccount extends UserCommon{
    public function index(){
        ...
        if($this->login_user['state'] == 89){
            $users_id = $this->login_user['id'];
            $subordinate = Db::table('users')->where('id',$users_id)
                ->value('subordinate');
            $where10['s.uid'] = ['in',$subordinate];
        }
        ...
        $sum_money = Db::table('top_account_assets')->alias('s')->where($where10)->where($where12)->sum('money');
        $where = '';
        if($this->login_user['state'] == 90){
            $where['id'] = ['<',3];
