...
class Withdrawal extends UserCommon{
    public function index(){
    	...
    	$order_data = Db::table('withdrawal')
            ->alias('w')
            ->field('w.*,u.merchant_cname,ta.show_name')
            ->join('users u','u.id=w.uid')
            ->join('top_account_assets ta','ta.id=w.pay_type')
            ->where($where1)
            ->where($where2)
            ->where($where3)
            ->where($where4)
    ...
    public function add_withdrawal(){
        if (request()->isPost()) {
            ...
            $sql_trading_pass = Db::table("users")->where('id',$post_data['uid'])->value('trading_pass');

            if (encryption($post_data['trading_pass'])!==$sql_trading_pass) {
                $this->error('密码错误');
            }
            ...
            $result_data = $pay_Model->index($post_data,$user_account_data,$sql_name);
        ...
        $user_fee_data = Db::table('user_fee')
            ->alias('uf')
            ->field('uf.money,ta.show_name,uf.taid')
